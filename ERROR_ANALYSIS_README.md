...
1. По `adminAnswer` запускается локальный векторный поиск TF-IDF по `combined_data`.
2. Поиск возвращает `topK` наиболее близких текстовых источников.
3. Агент сравнивает найденные источники с `agentSources` и самим `agentAnswer`.
4. Если задан API-ключ LLM, найденные источники и базовый анализ дополнительно отправляются в модель, которая формулирует итоговое описание ошибки в JSON.
5. Если API-ключа нет, используется локальная логика сравнения без внешней LLM.
6. На выходе формируется описание причины ошибки:
